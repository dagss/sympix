...
def find_optimal_ring_lengths(n_start, optimal_lengths, possible_increments, undersample=False):
    """
    ...
    """
    def cost_function(length, opt_length):
        ...
        for prev_len, (prev_cost, prev_prev_len) in G[i - 1].items():
            for inc in possible_increments:
                ...
                if undersample and ringlen > optimal_lengths[i]:
                    continue
                if not undersample and ringlen < optimal_lengths[i]:
                ...
                cost = prev_cost + cost_function(ringlen, optimal_lengths[i])
...
def make_sympix_grid(nrings_min, k, undersample=False):
    """
    ...
    cost, tile_counts = find_optimal_ring_lengths(n_start, ntiles_optimal_arr, POSSIBLE_INCREMENTS,
