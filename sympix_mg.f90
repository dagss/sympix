...
contains
  ...
  subroutine rescale_bl_s(bl, rescaled_bl, lmax) bind(c, name='sympix_mg_rescale_bl_s')
  ...
  ! rescale_bl_s on the bls. Recursion factors recfac should be provided, call
  ! legendre_transform_recfac_s to get them.
  !
  ...
  subroutine compute_YDYt_block(n_D1, n_D2, dphi_D1, dphi_D2, &
    ...
    real(sp), dimension(n_D1, n_D1, n_D2, n_D2) :: out
    real(sp), dimension(lmax + 1) :: rescaled_bl, recfac
    !--
    integer(i4b) :: i1, j1, i2, j2
    real(dp) :: phi1, phi2
    real(dp), dimension(3) :: v1, v2
    real(sp), allocatable, dimension(:, :, :, :) :: xs
    ...
    call legendre_transform_s(rescaled_bl, recfac, lmax, xs, out, n_D1**2 * n_D2**2)
  ...
  subroutine compute_many_YDYt_blocks(&
    ...
    real(sp), dimension(lmax + 1) :: bl
    real(sp), dimension(tilesize1, tilesize1, tilesize2, tilesize2, nblocks) :: out_blocks
    integer(i4b) :: ierr  ! out-argument
    !--
    real(sp), dimension(:), allocatable :: rescaled_bl, recfac
    ...
    call rescale_bl_s(bl, rescaled_bl, lmax)
